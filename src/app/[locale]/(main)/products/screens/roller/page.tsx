...
const products = [
  { id: 'roller-vertical', name: 'Rolhor Verticaal', description: 'Oprolbaar in verticale cassette.', price: 129, features: ['Verticaal', 'Oprolbaar', 'Cassette'] },
  { id: 'roller-horizontal', name: 'Rolhor Horizontaal', description: 'Oprolbaar in horizontale cassette.', price: 149, features: ['Horizontaal', 'Oprolbaar', 'Cassette'] },
];

export default function RollerScreensPage() {
  return (
      ...
      <section className="bg-secondary dark:bg-bg-dark-2 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center text-white">
            ...
            <h1 className="font-display text-4xl md:text-5xl font-bold mb-4">Rolhorren</h1>
            <p className="text-lg text-gray-300 max-w-2xl mx-auto">Oprolbare horren die netjes uit het zicht verdwijnen.</p>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {products.map((product) => (
              <motion.div key={product.id} className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all" whileHover={{ y: -8 }}>
                <div className="h-48 overflow-hidden"><img alt={product.name} className="w-full h-full object-cover" src="/images/nav/horren-roller.png" /></div>
                <div className="p-6">
