...
export default function TrustWidget() {
    ...
    return (
        <>
            {/* Desktop: Sidebar Button/Badge (right side, vertically centered) */}
            <motion.div
            ...
            {/* Mobile: Bottom Button/Badge */}
            <motion.div
                className="flex md:hidden fixed bottom-4 right-4 z-40 bg-[#E5006C] shadow-lg rounded-xl p-2 border border-white/20 flex-row items-center gap-2 cursor-pointer transition-all"
                onClick={() => setIsOpen(!isOpen)}
                initial={{ y: 50, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 1 }}
            >
                <div className="w-7 h-7 rounded-full bg-white flex items-center justify-center text-[#E5006C] font-bold text-sm shadow-sm">
                    <i className="fas fa-check"></i>
                </div>
                <div className="w-7 h-7 rounded-full bg-white flex items-center justify-center text-[#E5006C] font-bold text-[10px] shadow-sm">
                    9,6
                </div>
            </motion.div>

            {/* Desktop: Expanded Card (appears to the left) */}
            <AnimatePresence>
                {isOpen && (
                    <>
                        {/* Desktop popup */}
